...
#ifndef SIG_FALCON1024_avx2_DISABLED
...
SIG_FALCON1024_avx2_sign_keypair(unsigned char *pk, unsigned char *sk, unsigned char *seed, unsigned long long seedlen)
{
	...
	sk[0] = 0x50 + 10;
	u = 1;
	v = Zf(trim_i8_encode)(sk + u, SIG_FALCON1024_avx2_SECRETKEYBYTES - u,
	...
	u += v;
	if (u != SIG_FALCON1024_avx2_SECRETKEYBYTES) {
		return -1;
	}
	 ...
	 */
	pk[0] = 0x00 + 10;
	v = Zf(modq_encode)(pk + 1, SIG_FALCON1024_avx2_PUBLICKEYBYTES - 1, h, 10);
	if (v != SIG_FALCON1024_avx2_PUBLICKEYBYTES - 1) {
		return -1;
	}
...
int
SIG_FALCON1024_avx2_sign_signature(unsigned char *sig, unsigned long long *siglen,
	...
	const unsigned char *sk)
{
	...
	TEMPALLOC unsigned char esig[SIG_FALCON1024_avx2_BYTES - 2 - sizeof nonce];
	...
	v = Zf(trim_i8_decode)(f, 10, Zf(max_fg_bits)[10],
		sk + u, SIG_FALCON1024_avx2_SECRETKEYBYTES - u);
	if (v == 0) {
		return -1;
	}
	u += v;
...
int
SIG_FALCON1024_avx2_sign_verify(const unsigned char *m, unsigned long long mlen,
	...
	const unsigned char *pk)
{
	...
	if (Zf(modq_decode)(h, 10, pk + 1, SIG_FALCON1024_avx2_PUBLICKEYBYTES - 1)
		!= SIG_FALCON1024_avx2_PUBLICKEYBYTES - 1)
	{
		return -1;
	}
